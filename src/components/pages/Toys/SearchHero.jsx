...
const SearchHero = () => {
    ...
    return (
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12  text-center ">
            <p className="mx-auto -mt-4 max-w-2xl text-lg tracking-tight text-slate-700 sm:mt-6">Welcome to
                <span className="border-b ml-2 border-dotted border-slate-300">Zooming Wheels</span>
            </p>
            ...
            <h1 className="mx-auto max-w-4xl font-display text-5xl font-medium tracking-tight text-slate-900 sm:text-6xl">
                <span className="inline-block">Your
                    <span className="relative mx-4 whitespace-nowrap text-blue-600">
                ...
                <span className="inline-block">Toys Store</span>
            </h1>

            <p className="mx-auto mt-3 max-w-2xl  text-lg tracking-tight text-slate-700 sm:mt-6">
                <span className="block">Search and get best toys for your kids</span>
                <span className="inline-block">We provide best toys</span>
            </p>

            <div className="mt-6 flex flex-col justify-center gap-y-5  sm:flex-row sm:gap-y-0 sm:gap-x-6">
                <Link to={user ? '/add-toy' : '/login'} className="group inline-flex items-center justify-center rounded-full py-2 px-4 text-sm font-semibold focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 bg-slate-900 text-white hover:bg-slate-700 hover:text-slate-100 active:bg-slate-800 active:text-slate-300 focus-visible:outline-slate-900 animate-fade-in-left"
                >
                    ...
                    <span className="ml-3">{user ? 'Add a toy ' : 'Login'}</span>
                </Link>
                <div className="relative flex flex-1 flex-col items-stretch sm:flex-none" data-headlessui-state="">
                    <button onClick={()=>{
                        ...
                        <span className="ml-3">{user ? 'My Toys' : 'Register'}</span>
                    </button>
                </div>
            </div>
